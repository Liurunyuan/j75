...
void InitECap1(void)
{
	   /*时钟使用系统时钟75MHz，捕获信号为电机转子位置传感器霍尔信号，
	    * 捕获信号不分频，捕获霍尔位置信号上升沿，当捕获信号周期大于17.46ms时，
	    * 将发生定时器周期大于霍尔信号周期，需要复杂的处理计算，这里约定此时电机转速为0*/
	   EALLOW;
	   SysCtrlRegs.PCLKCR1.bit.ECAP1ENCLK = 1;    /*eCAP1时钟允许*/
	   EDIS;
	   ...
	   ECap1Regs.ECCTL2.bit.CONT_ONESHT = 0; /* 配置为连续模式*/

	   ECap1Regs.ECCTL1.bit.CAP1POL = 0;          /* 1级事件捕捉上升沿*/
	   ECap1Regs.ECCTL1.bit.CTRRST1 = 0;          /* 1级事件捕捉后不清零计数器*/
	   ECap1Regs.ECCTL2.bit.SYNCI_EN = 0;         /* 屏蔽同步输入操作*/
	   ECap1Regs.ECCTL2.bit.SYNCO_SEL = 3;        /* 屏蔽同步信号输出*/
	   ECap1Regs.ECCTL1.bit.PRESCALE =0;           /*输入霍尔信号不进行分频*/
...
{
	EALLOW;
	SysCtrlRegs.PCLKCR1.bit.ECAP2ENCLK = 1;    /*eCAP1时钟允许*/
	/* Initialize eCAP1/2/3 */
	...
	/* Configure peripheral registers*/
	ECap2Regs.ECCTL2.bit.CONT_ONESHT = 10; /* 配置为连续模式*/
	ECap2Regs.ECCTL1.bit.CAP1POL = 0;          /* 1级事件捕捉上升沿*/
	ECap2Regs.ECCTL1.bit.CTRRST1 = 0;          /* 1级事件捕捉后不清零计数器*/
	ECap2Regs.ECCTL2.bit.SYNCI_EN = 0;         /* 屏蔽同步输入操作*/
	ECap2Regs.ECCTL2.bit.SYNCO_SEL = 3;        /* 屏蔽同步信号输出*/
	ECap2Regs.ECCTL1.bit.PRESCALE =0;           /*输入信号不进行分频*/
...
{
	   EALLOW;
	   SysCtrlRegs.PCLKCR1.bit.ECAP3ENCLK = 1;    /*eCAP1时钟允许*/
       /* Initialize eCAP1/2/3 */
	   ...
	   /* Configure peripheral registers*/
	   ECap3Regs.ECCTL2.bit.CONT_ONESHT = 10; /* 配置为连续模式*/
	   ECap3Regs.ECCTL1.bit.CAP1POL = 0;          /* 1级事件捕捉上升沿*/
	   ECap3Regs.ECCTL1.bit.CTRRST1 = 0;          /* 1级事件捕捉后不清零计数器*/
	   ECap3Regs.ECCTL2.bit.SYNCI_EN = 0;         /* 屏蔽同步输入操作*/
	   ECap3Regs.ECCTL2.bit.SYNCO_SEL = 3;        /* 屏蔽同步信号输出*/
	   ECap3Regs.ECCTL1.bit.PRESCALE =0;           /*输入信号不进行分频*/
